...
`Disqualified Reason Prompt`, `Lead Creation Redirect`, `Pipeline Section Visibility`,
`Workshop Datetime Modal`, `Organization Reload After Create`, the `Product Details` /
`Forecasting` scripts and the remaining `<style>` injectors. Nothing native replaces them
yet. They remain a standing risk: undiffable behaviour that can change between environments
without a deploy.
