...
`Disqualified Reason Prompt`, `Lead Creation Redirect`, `Workshop Datetime Modal`,
`Organization Reload After Create`, the `Product Details` script and the remaining
`<style>` injectors. Nothing native replaces them yet. They remain a standing risk:
